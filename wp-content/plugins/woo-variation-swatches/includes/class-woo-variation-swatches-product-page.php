	...
	class Woo_Variation_Swatches_Product_Page {
		...
		public function item_start( $data, $attribute_type, $variation_data = array() ) {
			...
			foreach($variation_ids as $variation)
			{
					...
					$price_vari = $variation_obj->get_price_html();
			...
			return sprintf( '<li %1$s class="variable-item %2$s-variable-item %2$s-variable-item-%3$s %4$s" title="%5$s" data-title="%5$s" data-value="%6$s" role="radio" tabindex="0"><div class="price_vari">'.$price_vari.'</div><div class="variable-item-contents">', wc_implode_html_attributes( $html_attributes ), esc_attr( $attribute_type ), esc_attr( $option_slug ), esc_attr( $css_class ), esc_html( $option_name ), esc_attr( $slug ) );
		}
