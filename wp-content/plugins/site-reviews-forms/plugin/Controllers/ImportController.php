...
use GeminiLabs\SiteReviews\Addon\Forms\Commands\Import;
...
class ImportController extends AddonController
{
    ...
    public function onImport(Request $request): void
    {
        if (!glsr()->can('import')) {
            glsr(Notice::class)->addError(
                _x('You do not have permission to import files.', 'admin-text', 'site-reviews-forms')
            );
            return;
        }
    ...
    public function registerImporter(): void
    {
        register_importer(
            ...
            _x('Import review forms from a JSON export file.', 'admin-text', 'site-reviews-forms'),
